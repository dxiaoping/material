...
//        }
    ...
    private List<TurnoverRes> parseTurnover1(List<ItemSaleDetail> itemSaleDetailList, String cycle) {
        ...
        BigDecimal saleVolume = new BigDecimal(0);
        long saleNum = 0l;
        ...
        for (ItemSaleDetail itemSaleDetail : itemSaleDetailList) {
            ...
            if (isInSection) {
                ...
                turnoverResList.add(turnoverRes);
                cost = getDetailCost(itemSaleDetail.getBatch(),itemSaleDetail.getItemId());
...
//        对最后一区间赋值
    ...
    private BigDecimal getDetailCost(String batch,String itemId) {
        BigDecimal cost = new BigDecimal(0);
        String[] batArr = batch.split(",");
        for (String co : batArr) {
            String supp = co.split("-")[0];
            String times = co.split("-")[1];
            String num = co.split("-")[2];
            RepoRemain repoRemain = repoRemainMapper.selectByItemId(itemId);
            RepoRemainDetailKey repoRemainDetailKey = new RepoRemainDetailKey();
            repoRemainDetailKey.setSupplierId(supp);
            repoRemainDetailKey.setTimes(Integer.valueOf(times));
            ...
            cost = cost.add(repoRemainDetail.getRepoRemainInPic().multiply(new BigDecimal(times)));
